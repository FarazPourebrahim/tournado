export type TourType = {
  value: "Relaxation" | "Cultural" | "Adventure" | "Luxury" | "Family" | "Nature" | "Historical" | "Sports"
  label: string
}

export type MockTour = {
  id: number
  title: string
  type: TourType
  location: string
  price: number
  duration: number
  guideAvailable: boolean
  image: string
  description: string
  highlights: string[]
  itinerary: {
    day: number
    title: string
    activities: string[]
  }[]
  included: string[]
  excluded: string[]
  requirements: string[]
  maxCapacity: number
  difficulty: "آسان" | "متوسط" | "سخت"
  bestTime: string
  transportation: string
  accommodation: string
}

export const mockTours: MockTour[] = [
  {
    id: 1,
    title: "تور کویر مرنجاب",
    location: "اصفهان",
    price: 700000,
    duration: 2,
    guideAvailable: true,
    type: { value: "Adventure", label: "ماجراجویی" },
    image: "/images/maranjab-desert.jpg",
    description:
        "تجربه‌ای فراموش‌نشدنی در قلب کویر مرنجاب با مشاهده غروب خیره‌کننده و شب‌های پرستاره. این تور شامل کمپ در کویر، غذای محلی و فعالیت‌های هیجان‌انگیز است.",
    highlights: [
      "مشاهده غروب و طلوع آفتاب در کویر",
      "کمپ زدن زیر آسمان پرستاره",
      "سافاری با خودروهای آفرود",
      "تجربه سکوت مطلق کویر",
      "عکاسی از مناظر بکر طبیعی",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به کویر و اسکان",
        activities: [
          "حرکت از اصفهان به سمت کویر مرنجاب",
          "ورود به کویر با خودروهای آفرود",
          "برپایی کمپ و آشنایی با محیط",
          "مشاهده غروب آفتاب",
          "شام و شب نشینی دور آتش",
        ],
      },
      {
        day: 2,
        title: "اکتشاف کویر و بازگشت",
        activities: [
          "مشاهده طلوع آفتاب",
          "صبحانه در کویر",
          "گشت و بازدید از نقاط دیدنی",
          "عکاسی از مناظر طبیعی",
          "بازگشت به اصفهان",
        ],
      },
    ],
    included: [
      "حمل و نقل با خودروی آفرود",
      "راهنمای مجرب کویر",
      "تجهیزات کمپینگ کامل",
      "وعده‌های غذایی (شام و صبحانه)",
      "بیمه مسافرتی",
    ],
    excluded: ["هزینه‌های شخصی", "نوشیدنی‌های سرد", "سوغات و خرید", "هزینه عکاسی حرفه‌ای"],
    requirements: ["داشتن کفش مناسب کوهنوردی", "لباس گرم برای شب", "کرم ضد آفتاب", "آب شخصی کافی"],
    maxCapacity: 12,
    difficulty: "متوسط",
    bestTime: "پاییز و زمستان",
    transportation: "خودروی آفرود",
    accommodation: "کمپ در کویر",
  },
  {
    id: 2,
    title: "تور تاریخی تخت جمشید",
    location: "شیراز",
    price: 15000000,
    duration: 3,
    guideAvailable: true,
    type: { value: "Historical", label: "تاریخی" },
    image: "/images/persepolis.jpg",
    description:
        "سفری به قلب تاریخ ایران باستان با بازدید از تخت جمشید، نقش رستم و سایر آثار تاریخی شیراز. همراه با راهنمای تاریخ‌شناس و توضیحات کامل.",
    highlights: [
      "بازدید از کاخ‌های تخت جمشید",
      "مشاهده نقوش سنگی باستانی",
      "بازدید از نقش رستم",
      "موزه ملی ایران",
      "باغ ارم و حافظیه",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به شیراز و بازدید شهری",
        activities: [
          "فرود در فرودگاه شیراز",
          "انتقال به هتل و اسکان",
          "بازدید از باغ ارم",
          "حافظیه و سعدیه",
          "شام در رستوران سنتی",
        ],
      },
      {
        day: 2,
        title: "تخت جمشید و نقش رستم",
        activities: [
          "صبحانه در هتل",
          "حرکت به سمت تخت جمشید",
          "بازدید کامل از کاخ‌های باستانی",
          "نقش رستم و مقبره‌های شاهان",
          "بازگشت به شیراز",
        ],
      },
      {
        day: 3,
        title: "بازدیدهای تکمیلی و بازگشت",
        activities: ["موزه پارس", "مسجد نصیرالملک", "بازار وکیل", "خرید سوغات", "انتقال به فرودگاه"],
      },
    ],
    included: [
      "بلیط هواپیما",
      "اقامت در هتل 4 ستاره",
      "راهنمای تاریخ‌شناس",
      "تمام وعده‌های غذایی",
      "حمل و نقل داخل شهر",
      "بلیط ورودی اماکن",
    ],
    excluded: ["هزینه‌های شخصی", "خرید سوغات", "نوشیدنی در رستوران‌ها"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی"],
    maxCapacity: 20,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "اتوبوس VIP",
    accommodation: "هتل 4 ستاره",
  },
  {
    id: 3,
    title: "تور آرامش چالوس",
    location: "مازندران",
    price: 12000000,
    duration: 4,
    guideAvailable: false,
    type: { value: "Relaxation", label: "آرامش" },
    image: "/images/chalus.jpg",
    description: "فرار از شلوغی شهر به سمت طبیعت بکر چالوس. تجربه آرامش کنار دریا، جنگل‌های سبز و هوای پاک کوهستان.",
    highlights: [
      "اقامت در ویلای لوکس کنار دریا",
      "پیاده‌روی در جنگل‌های سبز",
      "ماهیگیری در دریای کاسپین",
      "ماساژ درمانی و آرامش‌بخشی",
      "غذاهای محلی شمالی",
    ],
    itinerary: [
      {
        day: 1,
        title: "سفر به چالوس و اسکان",
        activities: [
          "حرکت از تهران",
          "توقف در جاده چالوس برای عکاسی",
          "ورود به ویلا و اسکان",
          "آشنایی با امکانات",
          "شام کنار دریا",
        ],
      },
      {
        day: 2,
        title: "فعالیت‌های دریایی",
        activities: [
          "صبحانه با منظره دریا",
          "شنا در دریای کاسپین",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "استراحت و آفتاب‌گیری",
        ],
      },
      {
        day: 3,
        title: "طبیعت‌گردی جنگلی",
        activities: [
          "پیاده‌روی در جنگل",
          "بازدید از آبشار محلی",
          "پیک‌نیک در طبیعت",
          "جمع‌آوری قارچ و گیاهان دارویی",
          "شب نشینی دور آتش",
        ],
      },
      {
        day: 4,
        title: "آرامش و بازگشت",
        activities: ["یوگا صبحگاهی", "ماساژ درمانی", "صبحانه دیرهنگام", "جمع‌آوری وسایل", "بازگشت به تهران"],
      },
    ],
    included: ["اقامت در ویلای لوکس", "تمام وعده‌های غذایی", "امکانات تفریحی ویلا", "ماساژ درمانی", "تجهیزات ماهیگیری"],
    excluded: ["حمل و نقل", "هزینه‌های شخصی", "نوشیدنی‌های الکلی", "فعالیت‌های اضافی"],
    requirements: ["لباس شنا", "کرم ضد آفتاب", "لباس راحت", "کفش پیاده‌روی"],
    maxCapacity: 8,
    difficulty: "آسان",
    bestTime: "تابستان",
    transportation: "شخصی",
    accommodation: "ویلای لوکس",
  },
  {
    id: 4,
    title: "تور فرهنگی بازار تبریز",
    location: "تبریز",
    price: 6000000,
    duration: 1,
    guideAvailable: true,
    type: { value: "Cultural", label: "فرهنگی" },
    image: "/images/tabriz-bazaar.jpg",
    description:
        "کشف فرهنگ غنی آذربایجان از طریق بازدید از بازار تاریخی تبریز، یکی از بزرگترین بازارهای سرپوشیده جهان.",
    highlights: [
      "بازدید از بازار تاریخی تبریز",
      "آشنایی با صنایع دستی محلی",
      "چشیدن غذاهای آذری",
      "خرید فرش و صنایع دستی",
      "بازدید از مسجد کبود",
    ],
    itinerary: [
      {
        day: 1,
        title: "کاوش در بازار تاریخی",
        activities: [
          "ورود به بازار تبریز",
          "بازدید از بخش‌های مختلف بازار",
          "آشنایی با تاجران محلی",
          "چشیدن چای آذری",
          "خرید سوغات و صنایع دستی",
          "بازدید از مسجد کبود",
        ],
      },
    ],
    included: ["راهنمای محلی", "ناهار سنتی آذری", "چای و شیرینی", "بلیط ورودی اماکن"],
    excluded: ["حمل و نقل", "خرید شخصی", "هزینه‌های اضافی"],
    requirements: ["کفش راحت پیاده‌روی", "لباس محتشم", "کیف پول نقدی"],
    maxCapacity: 15,
    difficulty: "آسان",
    bestTime: "تمام فصول",
    transportation: "پیاده",
    accommodation: "ندارد",
  },
  {
    id: 5,
    title: "تور کوه‌نوردی دماوند",
    location: "تهران",
    price: 20000000,
    duration: 5,
    guideAvailable: true,
    type: { value: "Adventure", label: "ماجراجویی" },
    image: "/images/damavand.jpg",
    description: "صعود به بلندترین قله ایران و خاورمیانه. تجربه‌ای چالش‌برانگیز برای کوهنوردان با تجربه.",
    highlights: [
      "صعود به قله دماوند (5610 متر)",
      "مشاهده دریاچه لار",
      "کمپ در ارتفاعات",
      "تجربه کوهنوردی حرفه‌ای",
      "مناظر خیره‌کننده البرز",
    ],
    itinerary: [
      {
        day: 1,
        title: "حرکت به پایگاه",
        activities: [
          "حرکت از تهران",
          "رسیدن به پلور",
          "اسکان در کلبه کوهنوردی",
          "آماده‌سازی تجهیزات",
          "استراحت و آماده‌سازی",
        ],
      },
      {
        day: 2,
        title: "صعود به کمپ یک",
        activities: ["صبحانه و آماده‌سازی", "شروع صعود", "رسیدن به کمپ یک (4200م)", "برپایی چادر", "استراحت و سازگاری"],
      },
      {
        day: 3,
        title: "صعود به کمپ دو",
        activities: [
          "ادامه صعود",
          "رسیدن به کمپ دو (5000م)",
          "اسکان در پناهگاه",
          "آماده‌سازی برای قله",
          "استراحت کوتاه",
        ],
      },
      {
        day: 4,
        title: "صعود به قله",
        activities: ["شروع صعود در نیمه شب", "رسیدن به قله دماوند", "عکاسی و جشن", "فرود به کمپ دو", "استراحت"],
      },
      {
        day: 5,
        title: "فرود و بازگشت",
        activities: ["فرود به پایگاه", "جمع‌آوری تجهیزات", "بازگشت به تهران", "جشن پایان تور"],
      },
    ],
    included: ["راهنمای کوهنوردی مجرب", "تجهیزات کوهنوردی", "اقامت در پناهگاه‌ها", "وعده‌های غذایی", "بیمه کوهنوردی"],
    excluded: ["تجهیزات شخصی", "لباس کوهنوردی", "هزینه‌های درمانی"],
    requirements: ["تجربه کوهنوردی قبلی", "آمادگی جسمانی بالا", "گواهی پزشکی", "تجهیزات شخصی کامل"],
    maxCapacity: 6,
    difficulty: "سخت",
    bestTime: "تابستان",
    transportation: "مینی‌بوس",
    accommodation: "پناهگاه کوهنوردی",
  },
  {
    id: 6,
    title: "تور آرامش جزیره کیش",
    location: "کیش",
    price: 25000000,
    duration: 7,
    guideAvailable: false,
    type: { value: "Relaxation", label: "آرامش" },
    image: "/images/kish-island.jpg",
    description:
        "سفری به جزیره کیش برای تجربه آرامش و لذت‌بخشی در بحرانی‌ترین جزیره ایران. این تور شامل بازدید از ساحل‌های زیبا، رستوران‌های سنتی و فعالیت‌های دریایی است.",
    highlights: [
      "بازدید از ساحل‌های زیبا کیش",
      "ماهیگیری در دریای کیش",
      "عکاسی از مناظر بحرانی",
      "رستوران‌های سنتی کیش",
      "بازی در آکواریوم",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به کیش و اسکان",
        activities: [
          "فرود در فرودگاه کیش",
          "انتقال به هتل و اسکان",
          "شام در رستوران سنتی",
          "عکاسی از ساحل‌های زیبا",
          "شادی ورودی",
        ],
      },
      {
        day: 2,
        title: "ساحل‌های کیش",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 3,
        title: "آکواریوم و بازارهای محلی",
        activities: [
          "بازدید از آکواریوم کیش",
          "پیاده‌روی در بازارهای محلی",
          "خرید سوغات و صنایع دستی",
          "شام در رستوران سنتی",
          "عکاسی از بازارهای محلی",
        ],
      },
      {
        day: 4,
        title: "ساحل‌های کیش",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 5,
        title: "ساحل‌های کیش",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 6,
        title: "ساحل‌های کیش",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 7,
        title: "فرود و بازگشت",
        activities: ["صبحانه با منظره دریا", "جمع‌آوری تجهیزات", "فرود به فرودگاه", "بازگشت به تهران", "جشن پایان تور"],
      },
    ],
    included: ["حمل و نقل با پیاده‌روی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 10,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "پیاده",
    accommodation: "هتل 3 ستاره",
  },
  {
    id: 7,
    title: "تور طبیعت‌گردی جنگل‌های گیلان",
    location: "گیلان",
    price: 9000000,
    duration: 2,
    guideAvailable: true,
    type: { value: "Adventure", label: "ماجراجویی" },
    image: "/images/gilan-forest.jpg",
    description:
        "سفری به جنگل‌های گیلان برای تجربه طبیعت بکر و کوهنوردی در مناطق زیبا. این تور شامل کمپ در جنگل، غذای محلی و فعالیت‌های هیجان‌انگیز است.",
    highlights: [
      "پیاده‌روی در جنگل‌های گیلان",
      "کمپ زدن در جنگل",
      "بازدید از آبشار محلی",
      "عکاسی از مناظر طبیعی",
      "تجربه کوهنوردی حرفه‌ای",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به گیلان و اسکان",
        activities: [
          "حرکت از تهران",
          "رسیدن به رشت",
          "اسکان در کلبه کوهنوردی",
          "آماده‌سازی تجهیزات",
          "استراحت و آماده‌سازی",
        ],
      },
      {
        day: 2,
        title: "کمپ در جنگل و بازگشت",
        activities: [
          "صبحانه و آماده‌سازی",
          "شروع پیاده‌روی در جنگل",
          "رسیدن به کمپ در جنگل",
          "برپایی چادر",
          "استراحت و سازگاری",
          "بازگشت به رشت",
        ],
      },
    ],
    included: [
      "حمل و نقل با خودروی شخصی",
      "راهنمای کوهنوردی مجرب",
      "تجهیزات کمپینگ کامل",
      "وعده‌های غذایی",
      "بیمه مسافرتی",
    ],
    excluded: ["تجهیزات شخصی", "نوشیدنی‌های سرد", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["داشتن کفش مناسب کوهنوردی", "لباس گرم برای شب", "کرم ضد آفتاب", "آب شخصی کافی"],
    maxCapacity: 12,
    difficulty: "متوسط",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "کمپ در جنگل",
  },
  {
    id: 8,
    title: "تور بازدید از ارگ بم",
    location: "کرمان",
    price: 8000000,
    duration: 1,
    guideAvailable: false,
    type: { value: "Historical", label: "تاریخی" },
    image: "/images/bam-citadel.jpg",
    description:
        "کشف ارگ بم، یکی از بزرگترین ارگ‌های جهان و یکی از معروف‌ترین آثار تاریخی کرمان. این تور شامل بازدید از ارگ، مسجد و بازار محلی است.",
    highlights: ["بازدید از ارگ بم", "مسجد ارگ بم", "بازار محلی کرمان", "عکاسی از مناظر بکر", "تجربه سکوت مطلق ارگ"],
    itinerary: [
      {
        day: 1,
        title: "ورود به کرمان و بازدید از ارگ",
        activities: [
          "فرود در فرودگاه کرمان",
          "انتقال به هتل و اسکان",
          "بازدید از ارگ بم",
          "مسجد ارگ بم",
          "بازار محلی کرمان",
          "شام در رستوران سنتی",
          "بازگشت به فرودگاه",
          "بازگشت به تهران",
        ],
      },
    ],
    included: ["حمل و نقل با خودروی شخصی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 15,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "هتل 3 ستاره",
  },
  {
    id: 9,
    title: "تور لوکس جزیره قشم",
    location: "قشم",
    price: 40000000,
    duration: 10,
    guideAvailable: true,
    type: { value: "Luxury", label: "لوکس" },
    image: "/images/qeshm-island.jpg",
    description:
        "سفری به جزیره قشم برای تجربه لوکس و لذت‌بخشی در بحرانی‌ترین جزیره ایران. این تور شامل بازدید از ساحل‌های زیبا، رستوران‌های سنتی و فعالیت‌های دریایی است.",
    highlights: [
      "بازدید از ساحل‌های زیبا قشم",
      "ماهیگیری در دریای قشم",
      "عکاسی از مناظر بحرانی",
      "رستوران‌های سنتی قشم",
      "بازی در آکواریوم",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به قشم و اسکان",
        activities: [
          "فرود در فرودگاه قشم",
          "انتقال به هتل و اسکان",
          "شام در رستوران سنتی",
          "عکاسی از ساحل‌های زیبا",
          "شادی ورودی",
        ],
      },
      {
        day: 2,
        title: "ساحل‌های قشم",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 3,
        title: "آکواریوم و بازارهای محلی",
        activities: [
          "بازدید از آکواریوم قشم",
          "پیاده‌روی در بازارهای محلی",
          "خرید سوغات و صنایع دستی",
          "شام در رستوران سنتی",
          "عکاسی از بازارهای محلی",
        ],
      },
      {
        day: 4,
        title: "ساحل‌های قشم",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 5,
        title: "ساحل‌های قشم",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 6,
        title: "ساحل‌های قشم",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 7,
        title: "ساحل‌های قشم",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 8,
        title: "ساحل‌های قشم",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 9,
        title: "ساحل‌های قشم",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
        ],
      },
      {
        day: 10,
        title: "فرود و بازگشت",
        activities: ["صبحانه با منظره دریا", "جمع‌آوری تجهیزات", "فرود به فرودگاه", "بازگشت به تهران", "جشن پایان تور"],
      },
    ],
    included: ["حمل و نقل با پیاده‌روی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 10,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "پیاده",
    accommodation: "هتل 5 ستاره",
  },
  {
    id: 10,
    title: "تور عشایری کهگیلویه و بویراحمد",
    location: "کهگیلویه و بویراحمد",
    price: 5000000,
    duration: 2,
    guideAvailable: true,
    type: { value: "Cultural", label: "فرهنگی" },
    image: "/images/nomad-tour.jpg",
    description:
        "سفری به مناطق عشایری کهگیلویه و بویراحمد برای تجربه زندگی سنتی و فرهنگ آذربایجان. این تور شامل بازدید از بازارهای محلی، مساجد و خانه‌های عشایری است.",
    highlights: [
      "بازدید از بازارهای محلی کهگیلویه",
      "مسجد امام حسن",
      "خانه‌های عشایری",
      "عکاسی از مناظر بکر",
      "تجربه زندگی سنتی",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به کهگیلویه و اسکان",
        activities: [
          "فرود در فرودگاه کهگیلویه",
          "انتقال به هتل و اسکان",
          "شام در رستوران سنتی",
          "عکاسی از مناظر زیبا",
          "شادی ورودی",
        ],
      },
      {
        day: 2,
        title: "بازدید از بازارهای محلی و مساجد",
        activities: [
          "صبحانه با منظره زیبا",
          "پیاده‌روی در بازارهای محلی",
          "خرید سوغات و صنایع دستی",
          "بازدید از مسجد امام حسن",
          "خانه‌های عشایری",
          "عکاسی از مناظر زیبا",
          "بازگشت به فرودگاه",
          "بازگشت به تهران",
        ],
      },
    ],
    included: ["حمل و نقل با خودروی شخصی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 15,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "هتل 3 ستاره",
  },
  {
    id: 11,
    title: "تور ماجراجویی غار علیصدر",
    location: "همدان",
    price: 17000000,
    duration: 3,
    guideAvailable: false,
    type: { value: "Adventure", label: "ماجراجویی" },
    image: "/images/alisadr-cave.jpg",
    description:
        "سفری به غار علیصدر برای تجربه کوهنوردی و بازدید از این غار جذاب. این تور شامل کمپ در کوه، غذای محلی و فعالیت‌های هیجان‌انگیز است.",
    highlights: [
      "پیاده‌روی در کوه‌های همدان",
      "کمپ زدن در کوه",
      "بازدید از غار علیصدر",
      "عکاسی از مناظر طبیعی",
      "تجربه کوهنوردی حرفه‌ای",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به همدان و اسکان",
        activities: [
          "حرکت از تهران",
          "رسیدن به همدان",
          "اسکان در کلبه کوهنوردی",
          "آماده‌سازی تجهیزات",
          "استراحت و آماده‌سازی",
        ],
      },
      {
        day: 2,
        title: "صعود به کمپ",
        activities: ["صبحانه و آماده‌سازی", "شروع صعود", "رسیدن به کمپ در کوه", "برپایی چادر", "استراحت و سازگاری"],
      },
      {
        day: 3,
        title: "بازدید از غار علیصدر و بازگشت",
        activities: [
          "صبحانه و آماده‌سازی",
          "بازدید از غار علیصدر",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به همدان",
          "بازگشت به تهران",
        ],
      },
    ],
    included: [
      "حمل و نقل با خودروی شخصی",
      "راهنمای کوهنوردی مجرب",
      "تجهیزات کمپینگ کامل",
      "وعده‌های غذایی",
      "بیمه مسافرتی",
    ],
    excluded: ["تجهیزات شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["داشتن کفش مناسب کوهنوردی", "لباس گرم برای شب", "کرم ضد آفتاب", "آب شخصی کافی"],
    maxCapacity: 12,
    difficulty: "متوسط",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "کمپ در کوه",
  },
  {
    id: 12,
    title: "تور تاریخی و فرهنگی اصفهان",
    location: "اصفهان",
    price: 12000000,
    duration: 4,
    guideAvailable: true,
    type: { value: "Cultural", label: "فرهنگی" },
    image: "/images/isfahan-cultural.jpg",
    description:
        "سفری به اصفهان برای تجربه فرهنگ و تاریخ این شهر جدی. این تور شامل بازدید از مسجد امام حسن، باغ ارم و موزه ملی ایران است.",
    highlights: [
      "بازدید از مسجد امام حسن",
      "باغ ارم و حافظیه",
      "موزه ملی ایران",
      "عکاسی از مناظر زیبا",
      "تجربه زندگی سنتی",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به اصفهان و اسکان",
        activities: [
          "فرود در فرودگاه اصفهان",
          "انتقال به هتل و اسکان",
          "شام در رستوران سنتی",
          "عکاسی از مناظر زیبا",
          "شادی ورودی",
        ],
      },
      {
        day: 2,
        title: "مسجد امام حسن و باغ ارم",
        activities: [
          "صبحانه با منظره زیبا",
          "بازدید از مسجد امام حسن",
          "باغ ارم و حافظیه",
          "عکاسی از مناظر زیبا",
          "بازگشت به هتل",
        ],
      },
      {
        day: 3,
        title: "موزه ملی ایران و بازارهای محلی",
        activities: [
          "صبحانه با منظره زیبا",
          "بازدید از موزه ملی ایران",
          "بازارهای محلی اصفهان",
          "خرید سوغات و صنایع دستی",
          "عکاسی از مناظر زیبا",
          "بازگشت به هتل",
        ],
      },
      {
        day: 4,
        title: "بازدید از بازارهای محلی و بازگشت",
        activities: [
          "صبحانه با منظره زیبا",
          "بازارهای محلی اصفهان",
          "خرید سوغات و صنایع دستی",
          "عکاسی از مناظر زیبا",
          "جمع‌آوری تجهیزات",
          "بازگشت به فرودگاه",
          "بازگشت به تهران",
        ],
      },
    ],
    included: ["حمل و نقل با خودروی شخصی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 15,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "هتل 4 ستاره",
  },
  {
    id: 13,
    title: "تور ارزان سواحل چابهار",
    location: "چابهار",
    price: 3000000,
    duration: 2,
    guideAvailable: false,
    type: { value: "Relaxation", label: "آرامش" },
    image: "/images/chabahar-beach.jpg",
    description:
        "سفری به سواحل چابهار برای تجربه آرامش و لذت‌بخشی در بحرانی‌ترین جزیره ایران. این تور شامل بازدید از ساحل‌های زیبا، رستوران‌های سنتی و فعالیت‌های دریایی است.",
    highlights: [
      "بازدید از ساحل‌های زیبا چابهار",
      "ماهیگیری در دریای چابهار",
      "عکاسی از مناظر بحرانی",
      "رستوران‌های سنتی چابهار",
      "بازی در آکواریوم",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به چابهار و اسکان",
        activities: [
          "فرود در فرودگاه چابهار",
          "انتقال به هتل و اسکان",
          "شام در رستوران سنتی",
          "عکاسی از ساحل‌های زیبا",
          "شادی ورودی",
        ],
      },
      {
        day: 2,
        title: "ساحل‌های چابهار",
        activities: [
          "صبحانه با منظره دریا",
          "پیاده‌روی در ساحل‌های زیبا",
          "ماهیگیری",
          "ناهار ماهی تازه",
          "عکاسی از مناظر بحرانی",
          "جمع‌آوری تجهیزات",
          "بازگشت به فرودگاه",
          "بازگشت به تهران",
        ],
      },
    ],
    included: ["حمل و نقل با پیاده‌روی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 15,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "پیاده",
    accommodation: "هتل 3 ستاره",
  },
  {
    id: 14,
    title: "تور قایق‌سواری در زاینده‌رود",
    location: "اصفهان",
    price: 8000000,
    duration: 1,
    guideAvailable: false,
    type: { value: "Relaxation", label: "آرامش" },
    image: "/images/zayandeh-rud.jpg",
    description:
        "سفری به زاینده‌رود برای تجربه قایق‌سواری و لذت‌بخشی در این دریاچه زیبا. این تور شامل بازدید از ساحل‌های زیبا، رستوران‌های سنتی و فعالیت‌های دریایی است.",
    highlights: [
      "بازدید از ساحل‌های زیبا زاینده‌رود",
      "قایق‌سواری در زاینده‌رود",
      "عکاسی از مناظر بحرانی",
      "رستوران‌های سنتی زاینده‌رود",
      "بازی در آکواریوم",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به اصفهان و اسکان",
        activities: [
          "فرود در فرودگاه اصفهان",
          "انتقال به هتل و اسکان",
          "شام در رستوران سنتی",
          "عکاسی از ساحل‌های زیبا",
          "شادی ورودی",
        ],
      },
      {
        day: 2,
        title: "قایق‌سواری در زاینده‌رود",
        activities: [
          "صبحانه با منظره دریا",
          "قایق‌سواری در زاینده‌رود",
          "بازدید از ساحل‌های زیبا",
          "ناهار در رستوران سنتی",
          "عکاسی از مناظر بحرانی",
          "جمع‌آوری تجهیزات",
          "بازگشت به فرودگاه",
          "بازگشت به تهران",
        ],
      },
    ],
    included: ["حمل و نقل با پیاده‌روی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 15,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "پیاده",
    accommodation: "هتل 3 ستاره",
  },
  {
    id: 15,
    title: "تور آفرود لوت",
    location: "کرمان",
    price: 35000000,
    duration: 7,
    guideAvailable: true,
    type: { value: "Adventure", label: "ماجراجویی" },
    image: "/images/loot-desert.jpg",
    description:
        "سفری به لوت برای تجربه کوهنوردی و بازدید از این کویر جذاب. این تور شامل کمپ در کوه، غذای محلی و فعالیت‌های هیجان‌انگیز است.",
    highlights: [
      "پیاده‌روی در کوه‌های لوت",
      "کمپ زدن در کوه",
      "بازدید از لوت",
      "عکاسی از مناظر طبیعی",
      "تجربه کوهنوردی حرفه‌ای",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به کرمان و اسکان",
        activities: [
          "حرکت از تهران",
          "رسیدن به کرمان",
          "اسکان در کلبه کوهنوردی",
          "آماده‌سازی تجهیزات",
          "استراحت و آماده‌سازی",
        ],
      },
      {
        day: 2,
        title: "صعود به کمپ یک",
        activities: ["صبحانه و آماده‌سازی", "شروع صعود", "رسیدن به کمپ یک", "برپایی چادر", "استراحت و سازگاری"],
      },
      {
        day: 3,
        title: "صعود به کمپ دو",
        activities: ["ادامه صعود", "رسیدن به کمپ دو", "اسکان در پناهگاه", "آماده‌سازی برای لوت", "استراحت کوتاه"],
      },
      {
        day: 4,
        title: "بازدید از لوت",
        activities: [
          "صبحانه و آماده‌سازی",
          "بازدید از لوت",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به کمپ دو",
        ],
      },
      {
        day: 5,
        title: "صعود به کمپ یک",
        activities: ["صبحانه و آماده‌سازی", "صعود به کمپ یک", "برپایی چادر", "استراحت و سازگاری"],
      },
      {
        day: 6,
        title: "صعود به کرمان",
        activities: ["صبحانه و آماده‌سازی", "صعود به کرمان", "جمع‌آوری تجهیزات", "بازگشت به فرودگاه", "بازگشت به تهران"],
      },
    ],
    included: [
      "حمل و نقل با خودروی شخصی",
      "راهنمای کوهنوردی مجرب",
      "تجهیزات کمپینگ کامل",
      "وعده‌های غذایی",
      "بیمه مسافرتی",
    ],
    excluded: ["تجهیزات شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["داشتن کفش مناسب کوهنوردی", "لباس گرم برای شب", "کرم ضد آفتاب", "آب شخصی کافی"],
    maxCapacity: 12,
    difficulty: "متوسط",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "کمپ در کوه",
  },
  {
    id: 16,
    title: "تور زمستانی اسکی در دیزین",
    location: "البرز",
    price: 30000000,
    duration: 5,
    guideAvailable: false,
    type: { value: "Adventure", label: "ماجراجویی" },
    image: "/images/dizin-ski.jpg",
    description:
        "سفری به دیزین برای تجربه اسکی و لذت‌بخشی در این کوهستان زیبا. این تور شامل کمپ در کوه، غذای محلی و فعالیت‌های هیجان‌انگیز است.",
    highlights: [
      "پیاده‌روی در کوه‌های دیزین",
      "کمپ زدن در کوه",
      "بازدید از دیزین",
      "عکاسی از مناظر طبیعی",
      "تجربه اسکی حرفه‌ای",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به دیزین و اسکان",
        activities: [
          "حرکت از تهران",
          "رسیدن به دیزین",
          "اسکان در کلبه کوهنوردی",
          "آماده‌سازی تجهیزات",
          "استراحت و آماده‌سازی",
        ],
      },
      {
        day: 2,
        title: "اکتشاف کوه‌های دیزین",
        activities: [
          "صبحانه و آماده‌سازی",
          "اکتشاف کوه‌های دیزین",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به کلبه",
        ],
      },
      {
        day: 3,
        title: "اکتشاف کوه‌های دیزین",
        activities: [
          "صبحانه و آماده‌سازی",
          "اکتشاف کوه‌های دیزین",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به کلبه",
        ],
      },
      {
        day: 4,
        title: "اکتشاف کوه‌های دیزین",
        activities: [
          "صبحانه و آماده‌سازی",
          "اکتشاف کوه‌های دیزین",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به کلبه",
        ],
      },
      {
        day: 5,
        title: "فرود و بازگشت",
        activities: ["صبحانه و آماده‌سازی", "جمع‌آوری تجهیزات", "بازگشت به فرودگاه", "بازگشت به تهران"],
      },
    ],
    included: [
      "حمل و نقل با خودروی شخصی",
      "راهنمای کوهنوردی مجرب",
      "تجهیزات کمپینگ کامل",
      "وعده‌های غذایی",
      "بیمه مسافرتی",
    ],
    excluded: ["تجهیزات شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["داشتن کفش مناسب کوهنوردی", "لباس گرم برای شب", "کرم ضد آفتاب", "آب شخصی کافی"],
    maxCapacity: 12,
    difficulty: "متوسط",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "کمپ در کوه",
  },
  {
    id: 17,
    title: "تور فرهنگی روستاهای ماسوله",
    location: "گیلان",
    price: 11000000,
    duration: 3,
    guideAvailable: true,
    type: { value: "Cultural", label: "فرهنگی" },
    image: "/images/masuleh-village.jpg",
    description:
        "سفری به روستاهای ماسوله برای تجربه زندگی سنتی و فرهنگ آذربایجان. این تور شامل بازدید از بازارهای محلی، مساجد و خانه‌های سنتی است.",
    highlights: [
      "بازدید از بازارهای محلی ماسوله",
      "مسجد ماسوله",
      "خانه‌های سنتی ماسوله",
      "عکاسی از مناظر زیبا",
      "تجربه زندگی سنتی",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به گیلان و اسکان",
        activities: [
          "حرکت از تهران",
          "رسیدن به گیلان",
          "اسکان در کلبه کوهنوردی",
          "آماده‌سازی تجهیزات",
          "استراحت و آماده‌سازی",
        ],
      },
      {
        day: 2,
        title: "بازدید از بازارهای محلی و مساجد",
        activities: [
          "صبحانه با منظره زیبا",
          "بازدید از بازارهای محلی ماسوله",
          "خرید سوغات و صنایع دستی",
          "بازدید از مسجد ماسوله",
          "خانه‌های سنتی ماسوله",
          "عکاسی از مناظر زیبا",
          "بازگشت به کلبه",
        ],
      },
      {
        day: 3,
        title: "فرود و بازگشت",
        activities: ["صبحانه و آماده‌سازی", "جمع‌آوری تجهیزات", "بازگشت به فرودگاه", "بازگشت به تهران"],
      },
    ],
    included: ["حمل و نقل با خودروی شخصی", "راهنمای محلی", "تمام وعده‌های غذایی", "بیمه مسافرتی"],
    excluded: ["حمل و نقل شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["کفش راحت پیاده‌روی", "لباس مناسب فصل", "کلاه آفتابی", "آب شخصی کافی"],
    maxCapacity: 15,
    difficulty: "آسان",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "کلبه کوهنوردی",
  },
  {
    id: 18,
    title: "تور زمستانی اسکی در دیزین",
    location: "البرز",
    price: 30000000,
    duration: 5,
    guideAvailable: false,
    type: { value: "Adventure", label: "ماجراجویی" },
    image: "/images/dizin-ski.jpg",
    description:
        "سفری به دیزین برای تجربه اسکی و لذت‌بخشی در این کوهستان زیبا. این تور شامل کمپ در کوه، غذای محلی و فعالیت‌های هیجان‌انگیز است.",
    highlights: [
      "پیاده‌روی در کوه‌های دیزین",
      "کمپ زدن در کوه",
      "بازدید از دیزین",
      "عکاسی از مناظر طبیعی",
      "تجربه اسکی حرفه‌ای",
    ],
    itinerary: [
      {
        day: 1,
        title: "ورود به دیزین و اسکان",
        activities: [
          "حرکت از تهران",
          "رسیدن به دیزین",
          "اسکان در کلبه کوهنوردی",
          "آماده‌سازی تجهیزات",
          "استراحت و آماده‌سازی",
        ],
      },
      {
        day: 2,
        title: "اکتشاف کوه‌های دیزین",
        activities: [
          "صبحانه و آماده‌سازی",
          "اکتشاف کوه‌های دیزین",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به کلبه",
        ],
      },
      {
        day: 3,
        title: "اکتشاف کوه‌های دیزین",
        activities: [
          "صبحانه و آماده‌سازی",
          "اکتشاف کوه‌های دیزین",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به کلبه",
        ],
      },
      {
        day: 4,
        title: "اکتشاف کوه‌های دیزین",
        activities: [
          "صبحانه و آماده‌سازی",
          "اکتشاف کوه‌های دیزین",
          "عکاسی از مناظر طبیعی",
          "جمع‌آوری تجهیزات",
          "بازگشت به کلبه",
        ],
      },
      {
        day: 5,
        title: "فرود و بازگشت",
        activities: ["صبحانه و آماده‌سازی", "جمع‌آوری تجهیزات", "بازگشت به فرودگاه", "بازگشت به تهران"],
      },
    ],
    included: [
      "حمل و نقل با خودروی شخصی",
      "راهنمای کوهنوردی مجرب",
      "تجهیزات کمپینگ کامل",
      "وعده‌های غذایی",
      "بیمه مسافرتی",
    ],
    excluded: ["تجهیزات شخصی", "نوشیدنی‌های الکلی", "سوغات و خرید شخصی", "هزینه عکاسی حرفه‌ای"],
    requirements: ["داشتن کفش مناسب کوهنوردی", "لباس گرم برای شب", "کرم ضد آفتاب", "آب شخصی کافی"],
    maxCapacity: 12,
    difficulty: "متوسط",
    bestTime: "بهار و پاییز",
    transportation: "خودروی شخصی",
    accommodation: "کلبه کوهنوردی",
  },
]
